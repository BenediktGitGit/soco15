...
4. The `d3.js` and `d3.min.js` files are built from de.soco.stockmarket.source files in the `src` directory. _Do not edit `d3.js` directly._ Instead, edit the de.soco.stockmarket.source files, and then run `make` to build the generated files.
